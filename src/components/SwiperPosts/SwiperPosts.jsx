                ...
                return (
                  <div className='row'>
                    <div className='posts' key={post.id}>
                      ...
                      <div>
                       <Link to={`/datapage/${post.id}`}>  <h2>{post.title}</h2></Link>
                        <div className='ditls_post'>
                     <p><FontAwesomeIcon icon={faUser} /> {post.name}</p>
                     <p className='rel'><FontAwesomeIcon icon={faBook} /> {post.type}</p>
                     <p><FontAwesomeIcon icon={faCalendarDays} /> {post.date}</p>
                        </div>
                        <div>
                          <p>{post.pragraph}</p>
                        </div>
             ...
             {/* <button onClick={()=>setcurent(page)}>{page}</button>  */}
              {
                Array.from({length:Nbpage},(_,i) => i+ 1 ).map(page =>{
                  return <Button variant="light"  onClick={()=>setcurent(page)}>{page}</Button>
